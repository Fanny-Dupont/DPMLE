...
#' @return Value of the  local approximation of the SCAD penalty, defined in Eq (19).
...
#' @param nbSteps Length of the time-series.
#' @param aInd First observation for each individual
#' @return Expectation of vector u, conditionally to the parameters from Eq (14)
...
#' @param nbSteps Length of the time-series.
#' @param aInd First observation for each individual
#' @return Expectation of vector u, conditionally to the parameters from Eq (15) for non-stationary HMM.
    ...
    if(animal < nbAnimals && t==(aInd[animal+1]-1)){
      ...
      for(i in (1:nbStates)){
        for(j in (1:nbStates)){
          ...
          elnV[t,i,j] <- elnalpha[t,i]+ltrMat[t+1,i,j]+lnProbs[t+1,j]+elnbeta[t+1,j]
          nn <- logspace_add( nn, elnV[t,i,j])
        }
      }
    }
...
#' @param nbSteps Length of the time-series.
#' @param aInd First observation for each individual
#' @return Expectation of vector u, conditionally to the parameters from Eq (14) non-stationary HMM.
  ...
  for(t in (1:(nbSteps-1))){
    ...
    if(animal < nbAnimals && t==(aInd[animal+1]-1)){
      ...
      for(i in (1:nbStates)){
        for(j in (1:nbStates)){
          ltrMat[i,j] = log(trMat[i,j])
          elnV[t,i,j] <- elnalpha[t,i]+ltrMat[i,j]+lnProbs[t+1,j]+elnbeta[t+1,j]
          nn <- logspace_add( nn, elnV[t,i,j])
        }
      }
    }
...
#' Compute the part of (II) of eq (13) that is not penalized by the SCAD. It is the same for stationary and non-stationary HMMs.
...
#' @param aInd First observation for each individual
#' @param elnU Expectation of vector u, conditionally to the parameters from Eq (15), obtained from logU function.
...
#' Optimizes Eq (13) with respect to the standard deviation of the gamma distribution.
...
#' @param nbSteps Length of time-series
#' @param aInd First observation for each individual
#' @param elnU Expectation of vector u, conditionally to the parameters from Eq (11), obtained from logU function.
#' @return Values for standard deviation of gamma distributions that maximizes Eq (13).
...
HMM.beta2tpm <- function(nbStates,beta,X,nbSteps)
{
  ...
  prod <- X%*%t(beta)
  # Take the exponential as in Eq (20) minus the maximum of the sum of the product to prevent from overflow
  qq <- exp(prod-max(prod))
  # Set off-diagonal elements of matrix, starting from 1st col and moving down
  for(t in 1:nbSteps){
    int <- tpm[t,,]
    diag(int) <- exp(-max(prod)) # since qq is eq (20) times exp(-max(prod)) was computed line 426. It has to be multiplied by exp(-max(prod)) everywhere.
...
#' Compute the part of (I) of eq (13) for non-stationary HMM (i.e., with an estimate of pi).
#' @param beta Regression coefficient for the covariates in the t.p.m
#' @param X Vector of covariates, the first column should be intercept. Hence ncol(X) = 1+ "Nb Covariates".
...
ComputeItpmNH<- function(beta,X,trMat,nbSteps, aInd,pi,lnProbs,Cn,elnV)
{
  ...
  for(t in (1:(nbSteps-1))){
    ...
    # Compute I frim eq (13)
    for(i in (1:nbStates)){
      lProbaState[t,i] = lalpha[t,i] + lbeta[t,i] +negllk
      for(j in (1:nbStates)){
        # Multiply eq (13) by exp(-b) to prevent from overflow
  ...
  # Multiply the other side of eq (13) by exp(-b), to obtain equivalent problem of maximization than maximizing eq (13) w respect to beta 
...
#' Compute the part of (I) of eq (13), stationary HMM.
#' @param ttrMat Working parameters for the t.p.m
#' @param nbSteps Length of time-series
#' @param aInd First observation for each individual
#' @param elnU Expectation of vector u, conditionally to the parameters from Eq (11), obtained from logU function.
#' @param elnV Expectation of vector u, conditionally to the parameters from Eq (12), obtained from logV function.
...
ComputetrMat <- function(ttrMat,nbSteps, aInd,elnU,elnV,nbStates,Cn)
{
  ...
  # Multiply the other side of eq (13) by exp(-b), to obtain equivalent problem of maximization than maximizing eq (13) w respect to ttrMat 
...
#' Compute the part of (II) of eq (13) that is penalized by the SCAD. Here: mean of step-length (mu) and concentration of turning angle (kappa)
...
#' @param aInd First observation for each individual
#' @param elnU Expectation of vector u, conditionally to the parameters from Eq (11), obtained from logU function.
#' @return (II) from eq (13) without the SCAD penalty function 
...
ComputeIIMuKappaNH <- function(lpar,lsd,etaP,pi,lambda,a,Obs, trMat,nbSteps, aInd,elnU){
  ...
  # Compute II & Multiply the other side of eq (13) by exp(-b), to obtain equivalent problem of maximization than maximizing eq (13) w respect to beta 
